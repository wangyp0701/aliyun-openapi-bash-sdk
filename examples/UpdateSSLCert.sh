...
source /opt/aliyun-openapi-bash-sdk/AliyunOpenApiSDK.sh
...
#查询证书id
#	aliapi_rpc GET cas.aliyuncs.com 2020-04-07 ListUserCertificateOrder --Keyword "$_domain" --OrderType upload|jq .CertificateOrderList.CertificateId


# 设置 DCDN 域名列表使用新的证书
for _domain in "${DOMAIN_LIST[@]}"; do
	aliapi_rpc GET dcdn.aliyuncs.com 2018-01-15 SetDcdnDomainSSLCertificate --DomainName "$_domain" --CertType upload --SSLProtocol on  --SSLPub "get_cert()" --SSLPri "get_key()"|| exit 103
    done
